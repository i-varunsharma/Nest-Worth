...
| Spending by category | `categorise`, summary SQL | `backend/src/lib/categorise.js`, `routes/transactions.js` | `statement.test.js`, `api.test.js` |
| Averages and running totals | SQL in `insights.js` | `backend/src/lib/insights.js` | `insights.test.js` |
...
|---|---|---|
| 400 | The body failed validation. The message says which rule. | `checkFamilyMember` in `shared/family.js`, or `backend/src/lib/validate.js` |
| 401 with `code: no_session` | Cookie missing or expired | `lib/sessions.js`, and `credentials: 'include'` in `lib/api.js` |
| 404 on an id | The row belongs to another user, or was deleted | The `WHERE id = ? AND user_id = ?` in the route |
| 429 | Rate limit | `lib/rateLimit.js` |
| 500 | A thrown error | The stack trace next to the request id |

---

## 3. A page shows an error or never loads

- Pages built on `loadFinances` stop and show the first failing request's
  message. That is deliberate. Check the Network tab for the red request.
...
## 4. The AI coach says something wrong
